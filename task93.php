...
<div class="cards">
<?php
$cars = [["maker"=>"Machine","model"=>"Carina","year"=>2001,"price"=>20000,"image"=>"https://a.d-cd.net/cea52es-480.jpg"],["maker"=>"Toyota","model"=>"Camry","year"=>2005,"price"=>30000,"image"=>"https://d1hu588lul0tna.cloudfront.net/toyotaone/ruru/002-camry-exclusive-next-steps_tcm-3020-870114.jpg"],["maker"=>"Audi","model"=>"A8","year"=>1986,"price"=>12000,"image"=>"http://www.theautohost.com/_contentPages/vehicleContentPages/audi/2017/A8%20L/images/2017-Audi-A8L-exterior-grille.jpg"],
...
if (isset($_REQUEST["maker"])) {
	if ($_REQUEST["maker"]=="Machine") {
		for($i = 0; $i < 8; $i++){
			if($cars[$i]["maker"] == $_REQUEST['maker']){
				$car_name = $cars[$i]["maker"] ."  ". $cars[$i]["model"];
				$price = $cars[$i]['price'];
				$year = 2019- $cars[$i]['year'];
				$img = $cars[$i]["image"];
				$car = "<div class='card'>
							<img width='120px' height='80px' src='$img'>
							<div class='description'> 
								<div class='title'>$car_name</div>
								<div class='price'> $price$</div>
								<div class='year'> $year years</div>
							</div>
						</div>";
				echo $car;
			}
	...
	if ($_REQUEST["maker"]=="Lada") {
		for($i = 0; $i < 8; $i++){
			if($cars[$i]["maker"] == $_REQUEST['maker']){
				...
				$year = 2019 - $cars[$i]['year'];
				$img = $cars[$i]["image"];
				$car = "<div class='card'><img width='170px' height='100px' src='$img'><div class='description'> 
				<div class='title'>$car_name</div>
				<div class='price'> $price$</div>
				<div class='year'> $year years</div>
				</div> </div>";
